...
export default function Mint() {
  ...
  // FOR MINTING
  const [how_many_EleFans, set_how_many_EleFans] = useState(1)
...
//
  ...
  async function mintEleFans(how_many_EleFans) {
    if (bananaContract) {
 
      const price = Number(bananaPrice)  * how_many_EleFans 

      const gasAmount = await bananaContract.methods.mintBoringBanana(how_many_EleFans).estimateGas({from: walletAddress, value: price})
      console.log("estimated gas",gasAmount)

      console.log({from: walletAddress, value: price})

      bananaContract.methods
            .mintBoringBanana(how_many_EleFans)
  ...
  return (
    <div id="bodyy" className="flex flex-col items-center justify-center min-h-screen py-2">
      <Head>
        <title>ELE FANS COLLECTION</title>
...
bananas out known to man." key="ogdesc" />
        ...
        <meta name="twitter:description" content="Here at EleFans Nft, we specialise in the world's finest digital bananas. We've put together a team spanning 3 continents, to bring you some of the most ‍NUTRITIOUS and DELICIOUS
      ...
      <div >
          <div className="flex items-center justify-between w-full border-b-2	pb-6">
            ...
            <nav className="flex flex-wrap flex-row justify-around Poppitandfinchsans">
              ...
              <a href="#roadmap" className="text-4xl text-white hover:text-black m-6">Faqs</a>
            ...
            <div className="flex flex-col items-center">
                ...
                <div id="mint" className="flex justify-around  mt-8 mx-6">
                  ...
                  <input 
                                      ...
                                      min="1"
                                      max="20"
                                      value={how_many_EleFans}
                                      onChange={ e => set_how_many_EleFans(e.target.value) }
                                      name="" 
                                      className="Poppitandfinchsans pl-4 text-4xl  inline bg-grey-lighter  py-2 font-normal rounded text-grey-darkest  font-bold"
                                  />
                  
                  <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none px-3 font-bold">ELEFANS!</span>
    
                </div>
                {saleStarted ? 
                <button onClick={() => mintEleFans(how_many_EleFans)} className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">MINT {how_many_EleFans} elefans for {(bananaPrice * how_many_EleFans) / (10 ** 18)} ETH + GAS</button>        
